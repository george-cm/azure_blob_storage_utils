...
def create_blob(filepath):
  ...
  try:
    ...
    block_blob_service = BlockBlobService(account_name=blobs['ezconfig']['storage_account_name'], account_key=blobs['ezconfig']['storage_account_key'])
...
def main():
  ...
  inputf = g.fileopenbox('Select the exe file to upload to Azure Storage', 'Azure Storage Uploader', default='./*.exe', filetypes=['*.exe'])
  ...
  url_file = Path(input_file.parent, dfn.create_proper_name_wo_suffix(input_file.stem) + '_url.log')
